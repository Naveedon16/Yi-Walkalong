...
export function RegistrationStatus() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Checked In': return 'text-[#6750a4] bg-[#f3edf7]  border-[#eaddff]';
      default: return 'text-[#49454f]  bg-[#f1f3f4]  border-[#cac4d0] ';
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto w-full">
      <div className="text-center mb-10 print:hidden">
        <h1 className="text-3xl font-bold text-[#1d1b20]  mb-4">View Registration Pass</h1>
        <p className="text-[#49454f] ">Enter your Registration ID, Phone, Institution ID or Institution Name to view your entry pass.</p>
      </div>

      <Card className="mb-8 border-[#cac4d0]  print:hidden">
      ...
      <div ref={resultContainerRef}>
        <AnimatePresence mode="wait">
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
          >
            <Card className="border-[#cac4d0] ">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6 border-b border-[#e1e2ec] ">
              ...
              <div className="bg-[#f8f9ff]  rounded-2xl p-6 flex flex-col items-center justify-center border border-[#e1e2ec] ">
        ...
        {searched && !isSearching && false && 0 > 0 && (
          ...
          >
            <Card className="border-[#cac4d0] ">
              <h3 className="text-xl font-bold text-[#1d1b20]  mb-4">{0} Institutions Found</h3>
              <p className="text-sm text-[#49454f]  mb-6">Select the correct institution to view its participants.</p>
              
              <div className="space-y-4">
                {([].map)((inst: any) => (
                  <div 
                    key={inst.institutionId} 
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-[#e1e2ec]  hover:bg-[#f8f9ff]  cursor-pointer transition-colors"
                    onClick={() => performSearch(inst.institutionId)}
                  >
                    <div className="mb-4 sm:mb-0">
                      <div className="font-bold text-[#1d1b20]  text-lg">{inst.institutionName || 'Unknown Institution'}</div>
                      <div className="text-sm text-[#49454f]  font-mono mb-1">{inst.institutionId}</div>
                      <div className="text-sm text-[#49454f] ">Coordinator: {inst.coordinatorName || 'N/A'}</div>
                    </div>
                    <div className="flex items-center justify-between sm:flex-col sm:items-end gap-2">
                      <div className="text-sm font-medium bg-[#f1f3f4]  px-3 py-1 rounded-full">
        ...
        {searched && !isSearching && results && results.length > 1 && !selectedResult && (
          ...
          >
            <Card className="border-[#cac4d0] ">
              <h3 className="text-xl font-bold text-[#1d1b20]  mb-4">{results.length} registrations found</h3>
              <div className="space-y-4">
                {results.map(r => (
                  <div 
                    key={r.id} 
                    className="flex items-center justify-between p-4 rounded-xl border border-[#e1e2ec]  hover:bg-[#f8f9ff]  cursor-pointer transition-colors"
                    onClick={() => setSelectedResult(r)}
                  >
                    <div>
                      <div className="font-bold text-[#1d1b20]  text-lg">{r.details.name}</div>
                      <div className="text-sm text-[#49454f]  font-mono">{r.id}</div>
                    </div>
        ...
        {searched && !isSearching && selectedResult && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
          >
            <Card id="registration-pass" className="border-t-4 border-t-[#6750a4] registration-pass bg-white ">
              ...
              <div className="flex flex-col items-center justify-center mb-6 pb-6 border-b border-[#e1e2ec] ">
                 <h2 className="text-2xl font-black text-[#6750a4] uppercase tracking-wider">Yi WalkAlong 2026</h2>
                 <p className="text-sm text-[#49454f]  mt-1">Official Registration Pass</p>
              </div>

              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6 border-b border-[#e1e2ec] ">
                <div>
                  <p className="text-sm text-[#49454f]  mb-1">Registration ID</p>
                  <div className="flex items-center gap-2">
                    <p className="text-xl font-bold text-[#1d1b20]  tracking-tight">{selectedResult.id}</p>
                    <button 
                      onClick={() => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                <div>
                  <p className="text-sm text-[#49454f] ">Name</p>
                  <p className="font-medium text-[#1d1b20] ">{selectedResult.details.name}</p>
                </div>
                <div>
                  <p className="text-sm text-[#49454f] ">Category</p>
                  <p className="font-medium text-[#1d1b20] ">{selectedResult.details.category}</p>
                </div>
                <div>
                  <p className="text-sm text-[#49454f] ">T-Shirt Size</p>
                  <p className="font-medium text-[#1d1b20] ">{selectedResult.details.tshirtSize}</p>
                </div>
                <div>
                  <p className="text-sm text-[#49454f] ">Phone</p>
                  <p className="font-medium text-[#1d1b20] ">{selectedResult.details.phone}</p>
                </div>
              </div>

              {selectedResult.status === 'Confirmed' && (
                <div className="bg-[#f8f9ff]  rounded-2xl p-6 flex flex-col items-center justify-center text-center border border-[#e1e2ec] ">
                  <div className="bg-white  p-3 rounded-xl shadow-sm mb-4">
                    <QrCode className="w-24 h-24 text-[#1d1b20] " />
                  </div>
                  <h4 className="font-bold text-[#1d1b20]  mb-1">Your Entry Pass is Ready</h4>
                  
                  <div className="w-full max-w-md my-6 text-left border-t border-[#e1e2ec]  pt-6">
                    <h5 className="font-bold text-[#1d1b20]  mb-4">How to check in</h5>
                    <div className="space-y-4">
                      <div className="flex gap-3">
                        <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#eaddff]  text-[#6750a4]  flex items-center justify-center text-sm font-bold">1</div>
                        <div>
                          <p className="font-medium text-[#1d1b20]  text-sm">Keep your pass ready</p>
                          <p className="text-sm text-[#49454f] ">Show this QR code when you arrive at the event.</p>
                          <div className="mt-4">
                    ...
                    <p className="text-xs text-[#79747e]  mt-4 italic text-center">Keep your registration pass accessible on your phone for faster check-in.</p>
                  </div>
                  ...
                  <p className="text-xs text-[#79747e]  print:hidden" data-html2canvas-ignore="true">Tip: If pass images are missing when printing, enable 'Background Graphics' in your browser's print settings.</p>
                </div>
              )}
              {selectedResult.status !== 'Confirmed' && (
        ...
        {searched && !isSearching && false && (
          ...
          >
            <Card className="border-[#cac4d0] ">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6 border-b border-[#e1e2ec] ">
                <div>
                  <h3 className="text-xl font-bold text-[#1d1b20] ">Institution Registration</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-sm text-[#49454f] ">Institution ID: {""}</p>
                    <button 
                      onClick={() => {
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-[#1d1b20] ">Total Participants: {0}</p>
                  <p className="text-xs text-[#79747e]  mt-1">Registered: {new Date("").toLocaleDateString()}</p>
                </div>
              </div>
              
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-sm text-left text-[#49454f] ">
                  <thead className="text-xs text-[#1d1b20]  uppercase bg-[#f1f3f4]  sticky top-0">
                    <tr>
                  ...
                  <tbody>
                    {[].map((p: any, idx: number) => (
                      <tr key={idx} className="border-b border-[#e1e2ec]  hover:bg-[#f8f9ff]  transition-colors">
                        <td className="px-4 py-3 font-medium text-[#1d1b20] ">{p.name}</td>
                        ...
                        <td className="px-4 py-3">
                          <span className="bg-[#f1f3f4]  px-2 py-1 rounded-md text-xs font-medium">{p.category}</span>
                        </td>
                        <td className="px-4 py-3">
              ...
              <div className="md:hidden space-y-4">
                {[].map((p: any, idx: number) => (
                  <div key={idx} className="bg-white  border border-[#e1e2ec]  rounded-xl p-4 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-[#1d1b20] ">{p.name}</h4>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(p.status || 'Confirmed')}`}>
                        {p.status || 'Confirmed'}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm mb-3">
                      <div><span className="text-[#79747e]  text-xs block">Age/Gender</span><span className="text-[#49454f] ">{p.age} / {p.gender}</span></div>
                      <div><span className="text-[#79747e]  text-xs block">T-Shirt</span><span className="text-[#49454f] ">{p.tshirtSize}</span></div>
                      <div><span className="text-[#79747e]  text-xs block">Phone</span><span className="text-[#49454f] ">{p.phone || '-'}</span></div>
                      <div><span className="text-[#79747e]  text-xs block">Category</span><span className="bg-[#f1f3f4]  px-2 py-0.5 rounded text-xs font-medium inline-block mt-0.5">{p.category}</span></div>
                    </div>
                    {p.email && (
                      <div className="text-sm"><span className="text-[#79747e]  text-xs block">Email</span><span className="text-[#49454f]  truncate block">{p.email}</span></div>
                    )}
                    <div className="mt-3">
        ...
        {searched && !isSearching && results && results.length === 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
          >
            <Card className="text-center py-12 border-[#cac4d0] ">
              <div className="w-16 h-16 rounded-full bg-[#f1f3f4]  flex items-center justify-center mx-auto mb-4">
                <Search className="w-8 h-8 text-[#79747e] " />
              </div>
              <h3 className="text-lg font-bold text-[#1d1b20]  mb-2">No Registration Found</h3>
              <p className="text-[#49454f]  mb-6">We couldn't find any registration matching "{query}". Please check the ID or Phone Number and try again.</p>
              <Button variant="outline" onClick={handleSearchAgain}>Search Again</Button>
            </Card>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showPrintConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => { setShowPrintConfirm(false); setTimeout(() => document.getElementById('print-pass-btn')?.focus(), 10); }}>
            ...
            >
              <div className="p-6">
                <h2 id="print-dialog-title" className="text-xl font-bold text-[#1d1b20]  mb-2">Ready to print?</h2>
                <p className="text-[#49454f]  mb-6">Make sure your registration pass and QR code are visible before continuing.</p>
