...
code = code.replace(
  /static async checkInParticipant\(registrationId: string\): Promise<any> \{/m,
...
code = code.replace(
  /const response = await apiClient\.post<any, any>\('checkInParticipant', \{/m,
  `let response;
    try {
